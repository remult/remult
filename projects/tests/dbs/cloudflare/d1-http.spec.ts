...
describe.skipIf(!CLOUDFLARE_ACCOUNT_ID || !CLOUDFLARE_API_TOKEN || !CLOUDFLARE_DATABASE_ID)('d1-http', () => {
  ...
  beforeAll(async () => {
    if (!CLOUDFLARE_ACCOUNT_ID || !CLOUDFLARE_API_TOKEN || !CLOUDFLARE_DATABASE_ID) {
      throw new Error('CLOUDFLARE_ACCOUNT_ID, CLOUDFLARE_API_TOKEN, CLOUDFLARE_DATABASE_ID not set')
    }

    d1DataProvider = new D1DataProvider(new D1HttpClient({
