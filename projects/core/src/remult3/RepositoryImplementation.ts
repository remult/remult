...
import { allEntities, Remult, isBackend, queryConfig as queryConfig, setControllerSettings, EventSource, EntityInfo, EntityInfoProvider } from "../context";
...
export class FieldRefImplementation<entityType, valueType> implements FieldRef<entityType, valueType> {
    ...
    async __performValidation() {
        try {
            if (this.settings.dbReadOnly !== true && this.settings.allowApiUpdate !== false) {
