...
export class entityForrawFilter extends EntityBase {
    @Fields.integer()
    id: number;
    static filter = Filter.createCustom<entityForrawFilter, {
        oneAndThree?: boolean;
        ...
        if (c.dbOneOrThree) {
            ...
            r.push(
                {
                    $and: [
                        SqlDatabase.rawFilter(async (x) => x.sql = e.id + ' in (' + x.addParameterAndReturnSqlToken(1) + "," + x.addParameterAndReturnSqlToken(3) + ")"),
    ...
    static oneAndThree = Filter.createCustom<entityForrawFilter>(() => ({ id: [1, 3] }));
    static testNumericValue = Filter.createCustom<entityForrawFilter, number>(( val) => ({ id: val }));
    static testObjectValue = Filter.createCustom<entityForrawFilter, { val: number; }>(( val) => ({ id: val.val }));
}
