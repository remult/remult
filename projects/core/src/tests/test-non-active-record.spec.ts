...
describe("test non active record", () => {
    ...
    it("test save of new row validation", async () => {
        ...
        try {
            ...
            expect((err as ErrorInfo<taskNonActiveRecord>)?.modelState?.title).toContain("empty");
    ...
    it("test save doesn't return non row elements", async () => {
        ...
        let item: taskNonActiveRecord & { changed?: boolean } = { ...await repo.insert({ title: 'test' }), title: 'test1', changed: true };
