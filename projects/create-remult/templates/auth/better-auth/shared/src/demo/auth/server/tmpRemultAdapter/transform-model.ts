...
import { remultIdField, transformField } from "./transform-field.js";
...
function transformModel(
  { modelName, fields }: ModelSchema,
  { useNumberId = false, getClassName, getTableName }: TransformSchemaOptions,
) {
  const className = getClassName(modelName);
  const entity = trimLines(`
	@Entity<${className}>('${getTableName(modelName)}', ${generateEntityProps(
    modelName,
  )})
  ...
  const transformedFields = Object.entries(fields)
    .map(([key, { fieldName, ...attrs }]) => ({
      fieldName: fieldName ?? key,
      ...attrs,
    })) // ensure there is a fieldName (some plugins don't define fully)
