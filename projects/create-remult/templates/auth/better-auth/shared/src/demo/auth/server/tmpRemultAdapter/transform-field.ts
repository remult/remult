...
export function transformField<T extends FieldType>(
  ...
  const transformedProps = transformFieldProps({
    required,
    defaultValue,
    type,
    unique,
  ...
  switch (type) {
    ...
    default:
      throw new Error(
        `Unimplemented field type: ${JSON.stringify({
          modelName,
          fieldName,
          type,
          defaultValue,
  ...
  if (references) {
    if (references.field !== "id") {
      ...
      throw new RemultBetterAuthError(
        `Model ${modelName} references a non-id field: ${JSON.stringify(
          references,
        )}`,
      );
    }
    ...
    field = `${field.trim()}
		@Relations.toOne<${fromClass}, ${toClass}>(() => ${toClass}, "${
      references.field
    }")
...
function transformFieldProps({
  ...
  function shouldAllowApiUpdate() {
    if (type === "date" && ["createdAt", "updatedAt"].includes(fieldName))
      return false;
